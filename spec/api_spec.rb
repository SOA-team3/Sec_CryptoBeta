...
require_relative '../app/models/calender_doc'
...
describe 'Test Calender Web API' do
  include Rack::Test::Methods

  before do
    # Wipe database before each test
    Dir.glob("#{Calender::STORE_DIR}/*.txt").each { |filename| FileUtils.rm(filename) }
  end
  ...
  describe 'Handle events' do
    it 'HAPPY: should be able to get list of all events' do
      Calender::Event.new(DATA[0]).save
      Calender::Event.new(DATA[1]).save
    ...
    it 'HAPPY: should be able to get details of a single event' do
      Calender::Event.new(DATA[1]).save
      id = Dir.glob("#{Calender::STORE_DIR}/*.txt").first.split(%r{[/.]})[3]
    ...
    it 'HAPPY: should be able to create new events' do
      ...
      post 'api/v1/calenders', DATA[1].to_json, req_header
